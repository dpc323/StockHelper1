...
@interface MonthViewController ()<YT_KeyBoardAccessoryViewDelegate, YT_KeyBoardViewDelegate,UITextFieldDelegate>
...
{
    NSString *param = [NSString stringWithFormat:@"code=cn_%@&start=%@&end=%@&period=m",code,@"20160101",[self getCurrentDate]];
...
- (void)keyBoardViewShouldConfirm:(YT_KeyBoardView *)keyBoardView {
    ...
    if (self.searchText.length == 6) {
        ...
        NSString *param = [NSString stringWithFormat:@"code=cn_%@&start=%@&end=%@&period=m",self.searchText,@"20160101",[self getCurrentDate]];
